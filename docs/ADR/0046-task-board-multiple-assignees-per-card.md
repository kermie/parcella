...
1. **Replace `Task.assigned_to_id` with a `TaskAssignee` join table**,
   ...
   Migration `0057_task_multiple_assignees` carries over each existing
...
**History note:** an earlier attempt at this feature (closed PR #113)
hit a migration-numbering collision with two other task-board PRs
(priority, tags) that landed on main first. This version is rebuilt
against current main with the join table renumbered to migration
`0057_task_multiple_assignees` (after `0056_task_tags`) rather than
reusing the original `0055`/`0056` numbering.
